  ...
  return (
    <main>
        ...
        <section className="hero">
          <div className="slideshow-container">
            {slides.map((slide, index) => (
              <div
                className={`mySlides ${index === currentSlide ? 'active' : ''}`}  // Fixed class name assignment
                key={index}
      ...
      <section className="decoration-by-occasion">
        <h2>{selectedCategory ? `Images for ${selectedCategory.charAt(0).toUpperCase() + selectedCategory.slice(1)}` : 'Decoration By Occasions'}</h2> {/* Fixed template literal */}
        {loading && <p>Loading...</p>}
        {!selectedCategory && (
