...
const App = () => {
  ...
  const fetchImages = async (searchTerm) => {
    ...
    try {
      ...
      const response = await fetch(`https://api.pexels.com/v1/search?query=${searchTerm}&per_page=5`, { // Fixed string interpolation
        headers: {
      ...
      setError(err.message); // Set error message for better user feedback
      setImages([]);
    } finally {
      setLoading(false);
    }
